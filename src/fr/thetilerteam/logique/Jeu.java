...
public class Jeu {
    ...
    private ArrayList<Carreau> carreauxJouables(Carte carte) {
    ...
    private void retirerCarreaux(char lettre) {
        carreauxListe.retirerCareauListe(lettre);
    }

    private void placerCarreau(char lettre, int positionX, int positionY, ArrayList<Carreau> listeCarreauJouable) {
    ...
    public String commande(Carte c, String chaine) throws Exception {
        ...
        if (test.equals("next")) {
            ...
            Carreau carreau = retrouverCarreau(test.charAt(0), carreauxJouables(c));
            // ON VERIFIE SI LA COMMANDE EST POSSIBLE
            if (!lettreJouable(carreauxJouables(c), test.charAt(0)))
                throw new Exception("Lettre pas dans la liste !");
            if (mur.peutRecevoir(X, Y, carreau))
                try {
                    placerCarreau(test.charAt(0), X - 1, Y - 1, carreauxJouables(c));
